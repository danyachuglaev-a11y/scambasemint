...
async def moderate_accept(callback: CallbackQuery):
    ...
    # Собираем фото и видео из сообщения админа
    all_photo_ids = []
    all_video_ids = []
    
    # Фото в текущем сообщении
    if callback.message.photo:
        for photo in callback.message.photo:
            all_photo_ids.append(photo.file_id)
    
    # Фото/видео в ответе на сообщение
    if callback.message.reply_to_message:
        reply = callback.message.reply_to_message
        if reply.photo:
            for photo in reply.photo:
                all_photo_ids.append(photo.file_id)
        if reply.video:
            all_video_ids.append(reply.video.file_id)
