    ...
    for admin_id in admin_ids:
        try:
            ...
            if photo_ids and photo_ids[0] != "no_screenshot":
                ...
                admin_message_ids.append(msg.message_id)
            ...
            for pid in photo_ids:
                if pid != "no_screenshot":
                    await db.execute(
                        ...
                        (admin_message_ids[0], "photo", pid)
                    )
            if video_id and video_id != "no_video":
                await db.execute(
                    "INSERT INTO report_media (admin_message_id, media_type, file_id) VALUES (?, ?, ?)",
                    (admin_message_ids[0], "video", video_id)
                )
            await db.execute("""
                INSERT INTO scammer_reports (admin_message_id, scammer_username, scammer_id, reason)
                VALUES (?, ?, ?, ?)
            """, (admin_message_ids[0], scammer_username, scammer_id, reason))
            await db.commit()
...
# ======================================================
# MODERATION (ВИДЕО ПЕРВЫМИ, ПОТОМ ФОТОАЛЬБОМ С ТЕКСТОМ)
# ======================================================
